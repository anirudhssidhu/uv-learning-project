...
shutil.rmtree('src/uv_demo.egg-info', ignore_errors=True);
for f in glob.glob('src/uv_demo/*.so'): os.remove(f);
for f in glob.glob('src/uv_demo/*.c'): os.remove(f);
"""
...
def tests_compiled(session: nox.Session) -> None:
    ...
    session.install("-e", ".", env={"UV_DEMO_COMPILE_MYPYC": "1"})
